    ...
    .then((orders) => {
      ...
      for (let i = 0; i < columnHeaders.length; i += 1) {
        ...
        for (let j = 0; j < orders.length; j += 1) {
          ...
          if (typeof orders[j][headerNames] === 'number' && headerNames !== 'megerkezett') {
            ws.cell(j + 2, i + 1).number(orders[j][headerNames]);
          } else if (typeof orders[j][headerNames] === 'number' && headerNames === 'megerkezett') {
          ...
          } else if (typeof orders[j][headerNames] === 'string' && headerNames !== 'allapot') {
      ...
      const date = new Date();
      const excelName = `kreativliget_rendelesek_${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}-${Date.now()}`;
